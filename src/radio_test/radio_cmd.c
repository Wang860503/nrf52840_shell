...
SHELL_STATIC_SUBCMD_SET_CREATE(
...
#if defined(RADIO_MODE_MODE_Nrf_250Kbit)
    SHELL_CMD(nrf_250Kbit, NULL, "250 kbit/s Nordic proprietary radio mode",
              cmd_nrf_250kbit),
#endif /* defined(RADIO_MODE_MODE_Nrf_250Kbit) */
    ...
    SHELL_CMD(ble_1Mbit, NULL, "1 Mbit/s Bluetooth Low Energy", cmd_ble_1mbit),
    SHELL_CMD(ble_2Mbit, NULL, "2 Mbit/s Bluetooth Low Energy", cmd_ble_2mbit),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD(pattern_random, NULL, "Set the transmission pattern to random.",
              cmd_pattern_random),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    sub_fem_antenna,
    SHELL_CMD(ant_1, NULL, "ANT1 enabled, ANT2 disabled.", cmd_fem_antenna_1),
    SHELL_CMD(ant_2, NULL, "ANT1 disabled, ANT2 enabled", cmd_fem_antenna_2),
    SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD(tx_power_control, NULL,
              "Set the front-end module (FEM) Tx power control specific to the "
              "FEM in use <tx_power_control>.",
...
SHELL_CMD_REGISTER(time_on_channel, NULL,
                   ...
                   cmd_time_set);
SHELL_CMD_REGISTER(cancel, NULL, "Cancel the sweep or the carrier", cmd_cancel);
...
SHELL_CMD_REGISTER(total_output_power, NULL,
                   "Total output power in dBm, including gain of the attached "
                   "front-end module. "
                   "<tx power> dBm",
                   cmd_total_output_power_set);
#endif /* CONFIG_RADIO_TEST_POWER_CONTROL_AUTOMATIC */

SHELL_CMD_REGISTER(transmit_pattern, &sub_transmit_pattern,
                   "Set the transmission pattern", cmd_transmit_pattern_set);
...
SHELL_CMD_REGISTER(parameters_print, NULL,
                   "Print current delay, channel and so on", cmd_print);
...
#endif
#if CONFIG_FEM
SHELL_CMD_REGISTER(fem, &sub_fem,
