...
int hci_uart_init(hci_uart_read_cb cb)
{
	...
	LOG_INF("Checking UART device readiness...");
	if (!device_is_ready(hci_uart_dev)) {
		LOG_ERR("UART device not ready");
		LOG_ERR("Device pointer: %p", hci_uart_dev);
		LOG_ERR("Please check:");
		LOG_ERR("  - Device tree configuration (ncs,dtm-uart)");
		LOG_ERR("  - UART driver is enabled");
		LOG_ERR("  - Correct overlay file is used for your board");
		return -EIO;
	}
