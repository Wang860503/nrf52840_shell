...
/* Track if wait timer has been initialized */
static bool wait_timer_initialized = false;
...
int dtm_uart_wait_init(void)
{
	...
	/* If timer is already initialized, skip initialization */
	if (wait_timer_initialized) {
		LOG_DBG("Wait timer already initialized, skipping init");
		return 0;
	}

	err = nrfx_timer_init(&wait_timer, &timer_cfg, wait_timer_handler);
	
	/* If timer was already initialized, just reconfigure it */
	/* The error code 0xBAD000C (195887116) means "already initialized" */
	if ((unsigned int)err == 0xBAD000C || 
	    (int)err == 195887116 || 
	    (unsigned int)err == (unsigned int)195887116 ||
	    err == (nrfx_err_t)0xBAD000C) {
		LOG_DBG("Wait timer already initialized, reusing existing timer");
		err = NRFX_SUCCESS;
		wait_timer_initialized = true;
	}
	...
	irq_connect_dynamic(WAIT_TIMER_IRQ, CONFIG_DTM_TIMER_IRQ_PRIORITY,
