...
#define CONNECTIONLESS_IQ_REPORT_MAX_SIZE            \
  (sizeof(struct hci_connectionless_iq_report_evt) + \
...
static int cte_set(uint8_t cte_len, uint8_t cte_type, uint8_t pattern_len,
                   uint8_t* pattern) {
...
static int base_cc_evt(uint16_t opcode, uint8_t status) {
  ...
  return hci_uart_write(H4_TYPE_EVT, (uint8_t*)&hdr, sizeof(hdr),
                        (uint8_t*)&tmp, hdr.len);
}
...
static void iq_report_evt(struct dtm_iq_data* iq_data) {
  ...
  err = hci_uart_write(H4_TYPE_EVT, (uint8_t*)&hdr, sizeof(hdr), (uint8_t*)&buf,
                       hdr.len);
  if (err) {
...
static int hci_rx_test(uint16_t opcode, const uint8_t* data) {
  ...
  switch (opcode) {
    ...
    case BT_HCI_OP_LE_ENH_RX_TEST:
      ...
      LOG_DBG("RX Test command: v2, chan: %d, phy: %d, mod: %d.", chan, phy,
              mod);
      break;
    ...
    case BT_HCI_OP_LE_RX_TEST_V3:
      ...
      LOG_DBG(
          "RX Test command: v3, chan: %d, phy: %d, mod: %d, cte_len: %d,"
  ...
  if ((cte_len != 0) && (phy != BT_HCI_LE_TX_PHY_CODED_S8) &&
...
static int hci_tx_test(uint16_t opcode, const uint8_t* data) {
  ...
  switch (opcode) {
    case BT_HCI_OP_LE_TX_TEST:
      ...
      LOG_DBG("TX Test command: v1, chan: %d, data_len: %d, payload %d.", chan,
              data_len, payload);
      break;
    ...
    case BT_HCI_OP_LE_ENH_TX_TEST:
      ...
      phy = params->v2.phy;
      LOG_DBG(
          "TX Test command: v2, chan: %d, data_len: %d, payload: %d, phy: %d.",
    ...
    case BT_HCI_OP_LE_TX_TEST_V3:
      ...
      pattern = params->v3.ant_ids;
      LOG_DBG(
          "TX Test command: v3, chan: %d, data_len: %d, payload: %d, phy: %d,"
    ...
    case BT_HCI_OP_LE_TX_TEST_V4:
      ...
      tmp = (struct bt_hci_cp_le_tx_test_v4_tx_power*)&(
          params->v4.ant_ids[pattern_len]);
      power = tmp->tx_power;
      LOG_DBG(
          "TX Test command: v3, chan: %d, data_len: %d, payload: %d, phy: %d,"
...
static void dtm_hci_put(struct net_buf* buf) { k_fifo_put(&hci_rx_queue, buf); }

int dtm_tr_init(void) {
  int err;

  LOG_INF("Initializing HCI UART transport...");
  err = hci_uart_init(dtm_hci_put);
  if (err) {
    LOG_ERR("Failed to initialize HCI over UART: %d", err);
    LOG_ERR("Check that:");
    LOG_ERR("  - UART device is properly configured in device tree");
    LOG_ERR("  - No other application is using the UART");
    LOG_ERR("  - Correct COM port is selected");
    return err;
  }

  LOG_INF("HCI UART initialized successfully");
  LOG_INF("Initializing DTM module...");
  err = dtm_init(iq_report_evt);
  if (err) {
    LOG_ERR("Failed to initialize DTM: %d", err);
    LOG_ERR("Check that:");
    LOG_ERR("  - Radio peripheral is available");
    LOG_ERR("  - Clock control is properly configured");
    LOG_ERR("  - Timer peripherals are available");
    return err;
  }

  LOG_INF("DTM module initialized successfully");
  LOG_INF("DTM transport ready for communication");
