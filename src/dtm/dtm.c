...
/* Delay time from triggering the task "ON" for SysCtrl to starting RADIO
 * (setting RADIO TASK RXEN or TXEN)
 */
...
#define ANOMALY_172_TIMER_IRQ \
  NRFX_CONCAT_3(TIMER, ANOMALY_172_TIMER_INSTANCE, _IRQn)
#define ANOMALY_172_TIMER_IRQ_HANDLER \
  NRFX_CONCAT_3(nrfx_timer_, ANOMALY_172_TIMER_INSTANCE, _irq_handler)
...
static int errata216_on_wait(void) {
  ...
  nrfx_timer_compare(
      &dtm_inst.timer, NRF_TIMER_CC_CHANNEL1,
      nrfx_timer_us_to_ticks(&dtm_inst.timer, HMPAN_216_DELAY_US), true);
  ...
  if (!err) {
    ...
    /* Wait for the TIMER to count the required delay before starting the
     * Radio*/
 ...
 * @return 0 if successful, otherwise a negative error code
 */
static int errata216_off(void) { return mbox_send_dt(&off_channel, NULL); }

/**
 * @brief Return to code execution after the required delay for errata HMPAN-216
 * has elapsed.
 */
...
#else
static int errata216_on_wait(void) { return 0; }

static int errata216_off(void) { return 0; }

static void errata216_release(void) { /* Do nothing */ }
#endif /* NRF54H20_ERRATA_216_PRESENT */
...
static uint8_t const dtm_prbs15_content[] = {
    };
...
static void antenna_radio_pin_config(void) {
  ...
  for (size_t i = 0; i < DTM_HW_MAX_DFE_GPIO; i++) {
    uint32_t pin_value = (pin[i] == DTM_HW_DFE_PSEL_NOT_SET)
                             ? DTM_HW_DFE_GPIO_PIN_DISCONNECT
                             : pin[i];

    nrf_radio_dfe_pattern_pin_set(NRF_RADIO, pin_value, i);
  }
}
...
static void radio_cte_reset(void) {
  ...
  NRF_RADIO->DFEMODE |=
      ((RADIO_DFEMODE_DFEOPMODE_Disabled << RADIO_DFEMODE_DFEOPMODE_Pos) &
...
static void radio_cte_prepare(bool rx) {
  if ((rx && (dtm_inst.cte_info.mode == DTM_CTE_MODE_AOA)) ||
    ...
    NRF_RADIO->DFECTRL1 |=
        (dtm_inst.cte_info.slot << RADIO_DFECTRL1_TSWITCHSPACING_Pos);
  }
  ...
  if (rx) {
    ...
    NRF_RADIO->DFEPACKET.MAXCNT = (uint16_t)sizeof(dtm_inst.cte_info.data);
  } else {
...
static int dtm_clock_init(void) {
  int err;
  int res;
...
/* Note: We cannot directly reset nrfx internal state because m_cb is static in nrfx_timer.c */
/* Instead, we handle the "already initialized" error in timer_init() by cleaning hardware */
/* and then re-initializing. The hardware state is what matters for functionality. */
...
static int timer_init(void) {
  ...
  /* If timer was already initialized by radio_test or DTM, deinitialize it first */
  /* radio_test_init() is called at system startup via SYS_INIT, so timer may
   * already be initialized */
  radio_test_deinit();

  /* Directly access timer registers to ensure clean state */
  /* We always clean hardware state directly, regardless of nrfx internal state */
#if defined(CONFIG_SOC_SERIES_NRF54HX)
  NRF_TIMER_Type* timer_reg = NRF_TIMER020;
#elif defined(CONFIG_SOC_SERIES_NRF54LX)
  NRF_TIMER_Type* timer_reg = NRF_TIMER10;
#else
  NRF_TIMER_Type* timer_reg = NRF_TIMER0;
#endif

  /* Stop timer */
  nrf_timer_task_trigger(timer_reg, NRF_TIMER_TASK_STOP);

  /* Clear all shorts */
  nrf_timer_shorts_disable(timer_reg, ~0);

  /* Disable all interrupts */
  nrf_timer_int_disable(timer_reg, ~0);

  /* Clear all events */
  nrf_timer_event_clear(timer_reg, NRF_TIMER_EVENT_COMPARE0);
  nrf_timer_event_clear(timer_reg, NRF_TIMER_EVENT_COMPARE1);
  nrf_timer_event_clear(timer_reg, NRF_TIMER_EVENT_COMPARE2);
  nrf_timer_event_clear(timer_reg, NRF_TIMER_EVENT_COMPARE3);

  /* Clear timer */
  nrf_timer_task_trigger(timer_reg, NRF_TIMER_TASK_CLEAR);

  /* Disable IRQ before reconnecting */
  irq_disable(DEFAULT_TIMER_IRQ);

  /* Now initialize timer for DTM */
  err = nrfx_timer_init(&dtm_inst.timer, &timer_cfg, dtm_timer_handler);
  if (err != NRFX_SUCCESS) {
    printk("nrfx_timer_init failed with error code: %d (0x%08x)\n", (int)err,
           (unsigned int)err);
    printk("Timer register address: %p\n", (void*)timer_reg);
    printk("DTM timer p_reg: %p\n", (void*)dtm_inst.timer.p_reg);
    return -EAGAIN;
  }
  irq_connect_dynamic(DEFAULT_TIMER_IRQ, CONFIG_DTM_TIMER_IRQ_PRIORITY,
...
static int anomaly_timer_init(void) {
  ...
  irq_connect_dynamic(ANOMALY_172_TIMER_IRQ,
                      CONFIG_ANOMALY_172_TIMER_IRQ_PRIORITY,
                      ANOMALY_172_TIMER_IRQ_HANDLER, NULL, 0);

  nrfx_timer_compare(
      &dtm_inst.anomaly_timer, NRF_TIMER_CC_CHANNEL0,
      nrfx_timer_ms_to_ticks(&dtm_inst.anomaly_timer, BLOCKER_FIX_WAIT_DEFAULT),
      true);
...
static int8_t dtm_radio_min_power_get(nrf_radio_mode_t radio_mode,
                                      uint16_t frequency) {
  return fem_tx_output_power_min_get(radio_mode, frequency);
}

static int8_t dtm_radio_max_power_get(nrf_radio_mode_t radio_mode,
                                      uint16_t frequency) {
  return fem_tx_output_power_max_get(radio_mode, frequency);
}

static int8_t dtm_radio_nearest_power_get(int8_t tx_power,
                                          nrf_radio_mode_t radio_mode,
                                          uint16_t frequency) {
  int8_t tx_power_floor =
      fem_tx_output_power_check(tx_power, radio_mode, frequency, false);
  int8_t tx_power_ceiling =
      fem_tx_output_power_check(tx_power, radio_mode, frequency, true);
  int8_t output_power;

  output_power =
      (abs(tx_power_floor - tx_power) > abs(tx_power_ceiling - tx_power))
          ? tx_power_ceiling
          : tx_power_floor;
...
static void radio_tx_power_set(uint8_t channel, int8_t tx_power,
  ...
  if (IS_ENABLED(CONFIG_DTM_POWER_CONTROL_AUTOMATIC)) {
     ...
     * Due to limitations of the DTM specification output power level set
     * command check Tx output power level for channel 0. That is why output Tx
     * power needs to be aligned for final transmission channel.
     */
    tx_power = dtm_radio_nearest_power_get(tx_power, radio_mode, frequency);
    (void)fem_tx_output_power_prepare(tx_power, &radio_power, radio_mode,
                                      frequency);
  }
#else
  ARG_UNUSED(channel);
...
static int radio_init(void) {
  ...
  nrf_radio_fast_ramp_up_enable_set(NRF_RADIO,
                                    IS_ENABLED(CONFIG_DTM_FAST_RAMP_UP));
...
*/
  ...
  *(volatile uint32_t*)0x5302C7B4 = 0x0406007E;
  ...
  /* Do not initialize Radio and interrupts here - wait for dtm_start() */
  dtm_inst.state = STATE_UNINITIALIZED;
  dtm_inst.packet_len = 0;

  return 0;
}

/* Start DTM - initialize Radio and connect interrupts dynamically */
int dtm_start(void) {
  int err;
  ...
  /* Connect radio interrupts dynamically. */
  irq_connect_dynamic(RADIO_IRQn, CONFIG_DTM_RADIO_IRQ_PRIORITY, radio_handler,
                      NULL, 0);
...
/* Stop DTM - deinitialize Radio and disconnect interrupts */
int dtm_stop(void) {
  ...
  /* Properly uninitialize timer using nrfx API */
  /* This will clean up both hardware state and nrfx internal state */
  if (dtm_inst.timer.p_reg != NULL) {
    /* Disable timer and clear its state before uninit */
    nrfx_timer_disable(&dtm_inst.timer);
    ...
    /* Now properly uninitialize timer - this will reset nrfx internal state */
    nrfx_timer_uninit(&dtm_inst.timer);
  }

#if NRF52_ERRATA_172_PRESENT
  /* Properly uninitialize anomaly timer using nrfx API */
  if (dtm_inst.anomaly_timer.p_reg != NULL) {
    nrfx_timer_disable(&dtm_inst.anomaly_timer);
    nrfx_timer_clear(&dtm_inst.anomaly_timer);
    irq_disable(ANOMALY_172_TIMER_IRQ);
    
    /* Now properly uninitialize timer - this will reset nrfx internal state */
    nrfx_timer_uninit(&dtm_inst.anomaly_timer);
  }
#endif /* NRF52_ERRATA_172_PRESENT */

  /* Reset radio and disable interrupts */
  radio_reset();
  ...
  /* Reset transport initialization state to allow re-initialization */
  extern void dtm_tr_reset_state(void);
  dtm_tr_reset_state();
...
static bool check_pdu(const struct dtm_pdu* pdu) {
  ...
  pdu_packet_type = (uint32_t)(pdu->content[DTM_HEADER_OFFSET] & 0x0F);
  ...
  if (dtm_inst.cte_info.mode != DTM_CTE_MODE_OFF) {
    ...
    memset(dtm_inst.cte_info.data, 0, sizeof(dtm_inst.cte_info.data));
...
#else
static void errata_172_handle(bool enable) { ARG_UNUSED(enable); }
#endif /* NRF52_ERRATA_172_PRESENT */
...
static void endpoints_clear(void) {
  if (atomic_test_and_clear_bit(&dtm_inst.endpoint_state,
                                ENDPOINT_FORK_EGU_TIMER)) {
...
static void radio_prepare(bool rx) {
  ...
  nrf_radio_shorts_set(
      ...
      NRF_RADIO_SHORT_READY_START_MASK |
          (dtm_inst.cte_info.iq_rep_cb ? NRF_RADIO_SHORT_ADDRESS_RSSISTART_MASK
                                       : 0) |
          (dtm_inst.cte_info.mode == DTM_CTE_MODE_OFF
               ? DTM_RADIO_SHORT_END_DISABLE_MASK
               : NRF_RADIO_SHORT_PHYEND_DISABLE_MASK));
#else
  nrf_radio_shorts_set(NRF_RADIO, DTM_RADIO_SHORT_READY_START_MASK |
                                      DTM_RADIO_SHORT_END_DISABLE_MASK);
#endif /* DIRECTION_FINDING_SUPPORTED */
  ...
  if (dtm_inst.fem.vendor_ramp_up_time == 0) {
    ...
    dtm_inst.fem.ramp_up_time = dtm_inst.fem.vendor_ramp_up_time;
  }
#endif /* CONFIG_FEM */
...
#if CONFIG_FEM
    radio_ppi_configure(rx, (NRF_TIMER_SHORT_COMPARE1_STOP_MASK |
...
static int dtm_vendor_specific_pkt(uint32_t vendor_cmd,
  ...
  switch (vendor_cmd) {
    ...
    case CARRIER_TEST_STUDIO:
      ...
      radio_ppi_configure(false, (NRF_TIMER_SHORT_COMPARE1_STOP_MASK |
                                  NRF_TIMER_SHORT_COMPARE1_CLEAR_MASK));
      (void)fem_power_up();
...
static uint32_t dtm_packet_interval_calculate(uint32_t test_payload_length,
  ...
  if (dtm_inst.cte_info.mode != DTM_CTE_MODE_OFF) {
    ...
    test_packet_length += dtm_inst.cte_info.time * NRF_CTE_TIME_IN_US;
  }
...
void dtm_setup_prepare(void) { dtm_test_done(); }
...
int dtm_setup_read_max_supported_value(enum dtm_max_supported parameter,
                                       uint16_t* max_val) {
  if (!max_val) {
    return -EINVAL;
  }
...
int dtm_setup_set_cte_mode(enum dtm_cte_type type, uint8_t time) {
  ...
  if ((time < CTE_LENGTH_MIN) || (time > CTE_LENGTH_MAX)) {
    return -EINVAL;
  }
  ...
  cte_info |=
      ((dtm_inst.cte_info.mode & CTEINFO_TYPE_MASK) << CTEINFO_TYPE_POS);
...
int dtm_setup_set_antenna_params(uint8_t count, uint8_t* pattern,
                                 uint8_t pattern_len) {
...
struct dtm_tx_power dtm_setup_set_transmit_power(
    enum dtm_tx_power_request power, int8_t val, uint8_t channel) {
  uint16_t frequency = radio_frequency_get(channel);
  const int8_t tx_power_min =
      dtm_radio_min_power_get(dtm_inst.radio_mode, frequency);
  const int8_t tx_power_max =
      dtm_radio_max_power_get(dtm_inst.radio_mode, frequency);
  ...
  switch (power) {
    ...
    case DTM_TX_POWER_REQUEST_VAL:
      if (val <= tx_power_min) {
        ...
        dtm_inst.txpower =
            dtm_radio_nearest_power_get(val, dtm_inst.radio_mode, frequency);
      }
...
int dtm_test_transmit(uint8_t channel, uint8_t length, enum dtm_packet pkt) {
  ...
  switch (dtm_inst.packet_type) {
    case DTM_PACKET_PRBS9:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_PRBS9;
      /* Non-repeated, must copy entire pattern to PDU */
      memcpy(dtm_inst.current_pdu->content + header_len, dtm_prbs9_content,
             dtm_inst.packet_len);
      break;

    case DTM_PACKET_0F:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0X0F;
    ...
    case DTM_PACKET_55:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0X55;
    ...
    case DTM_PACKET_PRBS15:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_PRBS15;
      /* Non-repeated, must copy entire pattern to PDU */
    ...
    case DTM_PACKET_FF:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0XFF;
    ...
    case DTM_PACKET_00:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0X00;
      /* Bit pattern 00000000 repeated */
      memset(dtm_inst.current_pdu->content + header_len,
             RFPHY_TEST_0X00_REF_PATTERN, dtm_inst.packet_len);
      break;

    case DTM_PACKET_F0:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0XF0;
      /* Bit pattern 11110000 repeated */
      memset(dtm_inst.current_pdu->content + header_len,
             RFPHY_TEST_0XF0_REF_PATTERN, dtm_inst.packet_len);
      break;

    case DTM_PACKET_AA:
      dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] = DTM_PDU_TYPE_0XAA;
      /* Bit pattern 10101010 repeated */
      memset(dtm_inst.current_pdu->content + header_len,
             RFPHY_TEST_0XAA_REF_PATTERN, dtm_inst.packet_len);
      break;
  ...
  if (dtm_inst.cte_info.mode != DTM_CTE_MODE_OFF) {
    dtm_inst.current_pdu->content[DTM_HEADER_OFFSET] |= DTM_PKT_CP_BIT;
  ...
  nrfx_timer_extended_compare(
      &dtm_inst.timer, NRF_TIMER_CC_CHANNEL0,
...
#if NRF52_ERRATA_172_PRESENT
  if (dtm_inst.anomaly_172_wa_enabled) {
    nrfx_timer_compare(&dtm_inst.anomaly_timer, NRF_TIMER_CC_CHANNEL0,
                       nrfx_timer_ms_to_ticks(&dtm_inst.anomaly_timer,
                                              BLOCKER_FIX_WAIT_DEFAULT),
                       true);
    nrfx_timer_compare(
        &dtm_inst.anomaly_timer, NRF_TIMER_CC_CHANNEL1,
        nrfx_timer_us_to_ticks(&dtm_inst.anomaly_timer, BLOCKER_FIX_WAIT_END),
        true);
    nrf_timer_event_clear(dtm_inst.anomaly_timer.p_reg,
                          NRF_TIMER_EVENT_COMPARE0);
    nrf_timer_event_clear(dtm_inst.anomaly_timer.p_reg,
...
static void anomaly_timer_handler(nrf_timer_event_t event_type, void* context) {
  switch (event_type) {
    case NRF_TIMER_EVENT_COMPARE0: {
      ...
      NRFX_IRQ_PENDING_CLEAR(nrfx_get_irq_number(dtm_inst.anomaly_timer.p_reg));
    } break;
    ...
    case NRF_TIMER_EVENT_COMPARE1: {
      ...
      nrf_timer_event_clear(dtm_inst.anomaly_timer.p_reg,
                            NRF_TIMER_EVENT_COMPARE1);
      /* Disable this event. */
      nrfx_timer_compare(&dtm_inst.anomaly_timer, NRF_TIMER_CC_CHANNEL1, 0,
                         false);
