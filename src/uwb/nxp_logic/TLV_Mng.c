...
// #define LOG_MODULE_NAME TlvMng
// LOG_MODULE_REGISTER(LOG_MODULE_NAME);

K_THREAD_STACK_DEFINE(tlv_mng_stack, TLV_MNG_STACK_SIZE);
static UWBOSAL_TASK_HANDLE mTlvMngHnd;
struct k_thread tlv_mng_thread_data;
K_MSGQ_DEFINE(my_tlv_queue, sizeof(phLibUwb_Message_t), 1, 4);
...
static void tlvMngTask(void* p1, void* p2, void* p3);
...
bool tlvMngInit(void) {
  ...
  if (!tlvMngQueue) {
    printk("Could not create queue tlvMngQueue");
    return FALSE;
  }
  ...
  k_tid_t tid = k_thread_create(&tlv_mng_thread_data, tlv_mng_stack,
                                K_THREAD_STACK_SIZEOF(tlv_mng_stack),
                                (k_thread_entry_t)tlvMngTask, NULL, NULL, NULL,
                                TLV_MNG_PRIO, 0, K_NO_WAIT);

  if (tid == NULL) {
    printk("Could not create tlvMng task\n");
    return FALSE;
  }

  /* 設定 Thread 名稱 (這對 Debug 很有幫助，會顯示在 SystemView 或 Log 中) */
  k_thread_name_set(tid, "TlvMng");

  /* 儲存 Handle (轉型為 void* 以相容 NXP 結構) */
  mTlvMngHnd = (void*)tid;

  return TRUE;
}

void tlvMngTask(void* p1, void* p2, void* p3) {
  printk("%s task start", __func__);
...
void handleTLV(uint8_t deviceId, uint8_t* data) {
  ...
  UserAccessoryConfigData_iOS_t UserConfigData_iOS = {0};
  uint8_t BLEmessage_iOS[1 + sizeof(UserAccessoryConfigData_iOS_t)];
  /* Keep the config data stored in the variable for multiple devices
     (sessions) to use As get device info can not be done if any session is
     active */
  ...
  switch (*data) {
    case kMsg_ConfigureAndStart: {
       ...
       *
       * Host can also use this API to set DebugParams as Well as
       * AppParams the following parameters are suported:
       * CIR_CAPTURE_WINDOW
       * ANT_CONFIG_RX
       * TX_ADAPTIVE_PAYLOAD_POWER
       *
       * To easily set the AppParams list, following macros have been
       * defined. UWB_SET_APP_PARAM_VALUE(Parameter, Value) AND
       * UWB_SET_APP_PARAM_ARRAY(Parameter, ArrayValue, Length)
       *
       ...
       *         ANTENNAE_CONFIGURATION_RX, &antennaeConfigurationRx[0],
       * sizeof(antennaeConfigurationRx)),
       * };
       *
       *
       * UwbApi_ConfigureData_iOS(data + 1,
       *         *(data + SHAREABLE_DATA_LENGTH_OFFSET) +
       * SHAREABLE_DATA_HEADER_LENGTH, &profileInfo,
       *         sizeof(SetVendorAppParamsList) /
       * sizeof(SetVendorAppParamsList[0]), &SetVendorAppParamsList[0],
       *         sizeof(SetDebugParamsList) /
       * sizeof(SetDebugParamsList[0]), &SetDebugParamsList[0]);
       *
       *                      OR
       *
      ...
      if (mDevice[deviceId] == kDev_android_c) {
        ...
        uwb_status = UwbApi_ConfigureData_Android(
            data + 1, SHAREABLE_DATA_HEADER_LENGTH_ANDROID, &profileInfo, 0,
            NULL, 0, NULL);
        ...
        /* Fill-in input structure with device role/type and device mac
         * address*/
        ...
        uwb_status =
            UwbApi_ConfigureData_iOS(data + 1,
                                     *(data + SHAREABLE_DATA_LENGTH_OFFSET) +
                                         SHAREABLE_DATA_HEADER_LENGTH,
                                     &profileInfo, 0, NULL, 0, NULL);
        if (uwb_status != UWBAPI_STATUS_OK) {
          LOG_E("Shareable data not configured");
        } else {
          LOG_D("Shareable data configured");
          ...
          // API is called to erase Service characteristic in GATT as
          // ranging is already started
          if ((UserConfigData_iOS.UwbConfigData.uwb_spec_ver_minor[0] ==
               0x01) &&
              (UserConfigData_iOS.UwbConfigData.uwb_spec_ver_minor[1] ==
               0x00)) {
    ...
    case kMsg_Initialize_iOS: {
      ...
      phOsalUwb_MemCopy(UserConfigData_iOS.customerSpecMajorVer,
                        SpecMajorVersion, sizeof(SpecMajorVersion));
      phOsalUwb_MemCopy(UserConfigData_iOS.customerSpecMinorVer,
                        SpecMinorVersion, sizeof(SpecMinorVersion));
      UserConfigData_iOS.preferedUpdateRate = kUpdateRate_UserInteractive;

      mDevice[deviceId] = kDev_iPhone_c;

      /* UWB related definitions */
      uwb_status = UwbApi_GetUwbConfigData_iOS(
          DEMO_DEVICE_ROLE, &(UserConfigData_iOS.UwbConfigData));
      if (uwb_status != UWBAPI_STATUS_OK) {
        LOG_E("UwbApi_GetUwbConfigData_iOS configuration failed");
      }
      /* Build BLE Message, to be build depending on the iOS application
       * message stream In example application, it contains Message ID +
       * Accessory configuration data */
      BLEmessage_iOS[0] = kRsp_AccessoryConfigurationData; /* Response ID */

      /* Warning: this copy works correctly because the UserConfigData_iOS
       * does not need padding, if more fields were to be added and don't
       * fit the memory alignment then extra bytes could be added and more
       * data than required would be sent. To fix that a serialize utils
       * function would be required or use the packed attribute
       */
      phOsalUwb_MemCopy(&BLEmessage_iOS[1], (uint8_t*)&UserConfigData_iOS,
                        sizeof(UserConfigData_iOS));

      mMacAddr[deviceId] =
          ((UserConfigData_iOS.UwbConfigData.device_mac_addr[0] << 8) &
           0xFF00) |
      ...
      if ((UserConfigData_iOS.UwbConfigData.uwb_spec_ver_minor[0] == 0x01) &&
        ...
        /* Spec 1.1 required to update GATT server
             Update the GATT server with the same BLEmessage (only
           removing Response ID that is not part of the original
           definition) */
        if (Update_Nearby(deviceId, sizeof(BLEmessage_iOS) - 1,
                          BLEmessage_iOS + 1) != 0) {
          LOG_E("%s failed to update the GATT server with the BLE msg",
                __FUNCTION__);
        }
      ...
      } else if ((UserConfigData_iOS.UwbConfigData.uwb_spec_ver_minor[0] ==
                  0x00) &&
                 (UserConfigData_iOS.UwbConfigData.uwb_spec_ver_minor[1] ==
                  0x00)) {
        LOG_I(" Following spec 1.0");
        /* Spec 1.0 support, clock drift not sent over BLE. BLE message
         * size must  */
        LOG_MAU8_I(
            "Sent over BLE: ", BLEmessage_iOS,
            sizeof(BLEmessage_iOS) - 2 /* clock drift not sent in spec 1.0 */);
    ...
    case kMsg_Initialize_Android: {
      ...
      mMacAddr[deviceId] =
          ((UwbDeviceConfigData.device_mac_addr[0] << 8) & 0xFF00) |
          ((UwbDeviceConfigData.device_mac_addr[1] << 0) & 0xFF);

      /* Build BLE Message, to be build depending on the Android
       * application message stream In example application, it contains
       * Message ID + Accessory configuration data */

      uint8_t BLEmessage_Android[1 + sizeof(UwbDeviceConfigData_t)];
      uint16_t dataLen = serializeUwbDeviceConfigData(&UwbDeviceConfigData,
                                                      &BLEmessage_Android[1]);
    ...
    switch (mSessionState[deviceId]) {
      case notStarted:
        ...
        if (operation == UWBAPI_STATUS_OK ||
            operation == UWBAPI_STATUS_SESSION_NOT_EXIST) {
...
bool handleSingleStopSession(uint8_t deviceId) {
  bool status = TRUE;

  LOG_D("Stopping session: %02X", mSessionHandle[deviceId]);
  UwbApi_StopRangingSession(mSessionHandle[deviceId]);
  mSessionState[deviceId] = notStarted;

  UwbApi_SessionDeinit(mSessionHandle[deviceId]);
  mSessionState[deviceId] = notCreated;
  return status;
}
