 ...
 * param pAppContext - [IN] pointer to AppContext_t which holds Array of
 AppStateInfo_t
 ...
 * \return FALSE otherwise
 */
BOOLEAN updateState(AppContext_t* pAppContext, uint32_t sessionId,
                    uint8_t state);
 ...
 * param pAppContext -[IN] pointer to AppContext_t which holds Array of
 * AppStateInfo_t param sessionIds -[OUT] array of sessionIds which contains
 * session IDs of ongoing Active sessions. Array should be able to hold elements
 * upto MAX_SESSIONS param pNoOfActiveSessions -[OUT] Count of active ranging
 * sessions, Zero if no ranging sessions are active
 * \return None
 */
void getActiveRangingSessionCount(AppContext_t* pAppContext,
                                  uint32_t sessionIds[],
                                  uint8_t* pNoOfActiveSessions);
 ...
 * \return None
 */
void getActiveSessionIds(AppContext_t* pAppContext, uint32_t sessionIds[],
                         uint8_t* pNoOfActiveSessions);
