...
EXTERNC void printPerParams(const phRfTestParams_t* pRfTestParams) {
  if (pRfTestParams != NULL) {
    ...
    NXPLOG_APP_I("TestRF : randomizedSize : %hu",
                 pRfTestParams->randomizedSize);
    NXPLOG_APP_I("TestRF : phrRangingBit : %" PRIu16,
                 pRfTestParams->phrRangingBit);
    NXPLOG_APP_I("TestRF : rmarkerRxStart : %" PRIu32,
                 pRfTestParams->rmarkerRxStart);
    NXPLOG_APP_I("TestRF : rmarkerTxStart : %" PRIu32,
                 pRfTestParams->rmarkerTxStart);
    NXPLOG_APP_I("TestRF : stsIndexAutoIncr : %hu",
...
EXTERNC void printLoopbackRecvData(
    const phTest_Loopback_Ntf_t* pRfTestRecvData) {
  if (pRfTestRecvData != NULL) {
    NXPLOG_APP_I("Loopback : status : %hu", pRfTestRecvData->status);
    NXPLOG_APP_I("Loopback : tx_ts : %" PRIu32 ".%" PRIu16,
                 pRfTestRecvData->tx_ts_int, pRfTestRecvData->tx_ts_frac);
    NXPLOG_APP_I("Loopback : rx_ts : %" PRIu32 ".%" PRIu16,
                 pRfTestRecvData->rx_ts_int, pRfTestRecvData->rx_ts_frac);
    NXPLOG_APP_I("Loopback : aoa_azimuth :  %d.%d",
                 TO_Q_9_7(pRfTestRecvData->aoa_azimuth));
    NXPLOG_APP_I("Loopback : aoa_elevation : %d.%d",
                 TO_Q_9_7(pRfTestRecvData->aoa_elevation));
    ...
    if (pRfTestRecvData->vs_data_len > 0) {
      ...
      NXPLOG_APP_I("PER : no_of_rx_antenna : %hu",
                   pRfTestRecvData->vs_data.no_of_rx_antenna);
      for (int i = 0; i < pRfTestRecvData->vs_data.no_of_rx_antenna; i++) {
        NXPLOG_APP_I("PER : rx_antenna_id : %hu",
                     pRfTestRecvData->vs_data.rx_antenna_id[i]);
      }
      for (int j = 0; j < pRfTestRecvData->vs_data.no_of_rx_antenna; j++) {
        NXPLOG_APP_I("PER : rssi_rx : %d.%d",
                     TO_Q_8_8(pRfTestRecvData->vs_data.rssi_rx[j]));
      }
      for (int k = 0; k < pRfTestRecvData->vs_data.no_of_rx_antenna; k++) {
        NXPLOG_APP_I("PER : snr_rx : %d.%d",
                     TO_Q_8_8(pRfTestRecvData->vs_data.snr_rx[k]));
      }
      NXPLOG_APP_I("PER : rx_cfo_est : %d.%d",
                   TO_Q_5_11(pRfTestRecvData->rx_cfo_est));
...
EXTERNC void printPerRecvData(const phTestPer_Rx_Ntf_t* pRfTestRecvData) {
  if (pRfTestRecvData != NULL) {
    ...
    NXPLOG_APP_I("PER : sync_cir_ready : %" PRIu32,
                 pRfTestRecvData->sync_cir_ready);
    NXPLOG_APP_I("PER : sfd_fail : %" PRIu32, pRfTestRecvData->sfd_fail);
    NXPLOG_APP_I("PER : sfd_found : %" PRIu32, pRfTestRecvData->sfd_found);
    NXPLOG_APP_I("PER : phr_dec_error : %" PRIu32,
                 pRfTestRecvData->phr_dec_error);
    NXPLOG_APP_I("PER : phr_bit_error : %" PRIu32,
                 pRfTestRecvData->phr_bit_error);
    NXPLOG_APP_I("PER : psdu_dec_error : %" PRIu32,
                 pRfTestRecvData->psdu_dec_error);
    NXPLOG_APP_I("PER : psdu_bit_error : %" PRIu32,
                 pRfTestRecvData->psdu_bit_error);
...
EXTERNC void printrxRecvData(const phTest_Rx_Ntf_t* pRfTestRecvData) {
  if (pRfTestRecvData != NULL) {
    ...
    NXPLOG_APP_I("TestRX : aoa_azimuth : %d.%d",
                 TO_Q_9_7(pRfTestRecvData->aoa_azimuth));
    NXPLOG_APP_I("TestRX : aoa_elevation : %d.%d",
                 TO_Q_9_7(pRfTestRecvData->aoa_elevation));
