 ...
 * - It means reading "header" first, then then reading the rest of the frame
 * later.
 *
 * @{
 */
...
typedef struct {
  ...
  /** This mutex is used to make tml interface read and write operations
   * mutually exclusive */
  void* mSyncMutex;
 ...
 *
 */
void uwb_uwbs_tml_switch_protocol(uwb_uwbs_tml_ctx_t* pCtx,
                                  uwb_uwbs_tml_mode_t mode);
...
#define SWITCH_PROTOCOL_SWUP \
  phTmlUwb_switch_protocol((uint8_t)kUWB_UWBS_TML_MODE_SWUP);
#define SWITCH_PROTOCOL_UCI \
  phTmlUwb_switch_protocol((uint8_t)kUWB_UWBS_TML_MODE_UCI);
#else
#define SWITCH_PROTOCOL_SWUP
#define SWITCH_PROTOCOL_UCI
#endif  // UWBIOT_TML_S32UART
 ...
 * @param[in,out]  pBufLen  Input: The max length that we can copy.  Output:
 * actual length read.
 *
 ...
 * @param[in,out]  pRxBufLen Input: The max length that we can copy.  Output:
 * actual length read.
 *
 ...
 * @return     Status of Tx/Rx
 */
UWBStatus_t uwb_uwbs_tml_helios_get_hdll_edl_ntf(uwb_uwbs_tml_ctx_t* pCtx,
                                                 uint8_t* pRxBuf,
                                                 size_t* pRxBufLen);
