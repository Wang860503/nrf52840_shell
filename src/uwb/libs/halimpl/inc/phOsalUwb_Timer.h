 ...
 *  already occupied
 *
 * @param[in] bAutoReload (not used in Posix) If bAutoReload is set to TRUE then
 * the timer will expire repeatedly with a frequency set by the
 * xTimerPeriodInTicks parameter. Timer restarts automatically. If bAutoReload
 * is set to FALSE then the timer will be a one-shot timer and enter the dormant
 ...
 *             Starts the requested, already created, timer If the timer is
 * already running, timer stops and restarts with the new timeout value and new
 * callback function in case any.
 *
 *
 ...
 * @param[in] pContext caller context, to be passed to the application callback
 * function
 *
 ...
 * @retval PH_OSALUWB_TIMER_START_ERROR timer could not be created due to system
 * error
 *
 **/
...
/**
 * @brief Stops already started timer, allows to stop running timer. In case
 * timer is stopped, timer callback will not be notified any more
 *
 * @param[in] dwTimerId  valid timer ID obtained during timer creation
 *
 * @retval UWBSTATUS_SUCCESS the operation was successful
 * @retval UWBSTATUS_NOT_INITIALISED  OSAL Module is not initialized
 * @retval UWBSTATUS_INVALID_PARAMETER invalid parameter passed to the function
 * @retval PH_OSALUWB_TIMER_STOP_ERROR  timer could not be stopped due to system
 * error
 *
 **/
