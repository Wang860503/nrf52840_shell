...
/* 1. 靜態定義 Stack 區域 (編譯器會自動對齊，不會佔用 Heap) */
K_THREAD_STACK_DEFINE(g_tml_reader_stack, TMLREADER_STACK_SIZE);

/* 2. 定義執行緒控制結構 */
struct k_thread g_tml_reader_thread;
...
static void phTmlUwb_PrintRecevedMessage(const uint8_t* const pBuffer,
...
UWBSTATUS phTmlUwb_Init(pphTmlUwb_Config_t pConfig) {
  ...
  /* Validate Input parameters */
    ...
    gpphTmlUwb_Context =
        (phTmlUwb_Context_t*)phOsalUwb_GetMemory(sizeof(phTmlUwb_Context_t));

    if (NULL == gpphTmlUwb_Context) {
      wInitStatus = PHUWBSTVAL(CID_UWB_TML, UWBSTATUS_FAILED);
    } else {
      /* Initialise all the internal TML variables */
      phOsalUwb_SetMemory(gpphTmlUwb_Context, PH_TMLUWB_RESET_VALUE,
      ...
      wInitStatus = phTmlUwb_open_and_configure(
      ...
      if (UWBSTATUS_SUCCESS != wInitStatus) {
        ...
        if (0 !=
            phOsalUwb_CreateSemaphore(&gpphTmlUwb_Context->rxSemaphore, 0)) {
          wInitStatus = UWBSTATUS_FAILED;
        } else if (0 != phOsalUwb_CreateSemaphore(
                            &gpphTmlUwb_Context->postMsgSemaphore, 0)) {
          wInitStatus = UWBSTATUS_FAILED;
        } else {
...
static UWBSTATUS phTmlUwb_StartThread(void) {
  ...
  pthread_create_status =
      phOsalUwb_Thread_Create((void**)&gpphTmlUwb_Context->readerThread,
                              &phTmlUwb_TmlReaderThread, &threadparams);
  if (0 != pthread_create_status) {
    wStartStatus = UWBSTATUS_FAILED;
    NXPLOG_UWB_TML_E("\n\r ---phTmlUwb_TmlReaderThread Task create failed ");
  } else {
    NXPLOG_UWB_TML_D("---phTmlUwb_TmlReaderThread Task create Success ");
  }
  // k_msleep(5);
  return wStartStatus;
}
...
static UWBSTATUS phTmlUwb_StartThread(void) {
  ...
  /* * 改用 Zephyr 原生 k_thread_create
   * 參數說明：
   * 1. &g_tml_reader_thread: 執行緒結構 (全域變數)
   * 2. g_tml_reader_stack: Stack 記憶體 (靜態定義)
   * 3. K_THREAD_STACK_SIZEOF(...): 自動計算大小
   * 4. phTmlUwb_TmlReaderThread: 入口函式
   * 5. NULL, NULL, NULL: 傳遞給入口函式的參數 (p1, p2, p3)
   * 6. 優先級
   * 7. 選項 (0)
   * 8. K_NO_WAIT: 立即啟動
   */
  tid = k_thread_create(&g_tml_reader_thread, g_tml_reader_stack,
                        K_THREAD_STACK_SIZEOF(g_tml_reader_stack),
                        (k_thread_entry_t)phTmlUwb_TmlReaderThread, NULL, NULL,
                        NULL, TMLREADER_PRIO, 0, K_NO_WAIT);

  if (tid == NULL) {
    wStartStatus = UWBSTATUS_FAILED;
    NXPLOG_UWB_TML_E("\n\r ---phTmlUwb_TmlReaderThread Task create failed ");
  } else {
    /* * 將 Zephyr 的 Thread ID 轉型存回 NXP 的 Context 結構中
     * 這樣其他函式 (如 Thread_Delete) 才能引用它
     */
    gpphTmlUwb_Context->readerThread = (void*)tid;

    NXPLOG_UWB_TML_D("---phTmlUwb_TmlReaderThread Task create Success ");

    /* 【建議】讓出 CPU，確保 Reader Thread 能完成初始化 */
    k_yield();
  }
...
static OSAL_TASK_RETURN_TYPE phTmlUwb_TmlReaderThread(void* pParam) {
  ...
  while (gpphTmlUwb_Context->bThreadDone) {
    ...
    if (phOsalUwb_ConsumeSemaphore_WithTimeout(gpphTmlUwb_Context->rxSemaphore,
                                               PH_TML_UWB_MAX_READER_WAIT) !=
        UWBSTATUS_SUCCESS) {
      ...
      if (NULL != gpphTmlUwb_Context->pDevHandle) {
        ...
        if (dwNoBytesWrRd > UCI_MAX_DATA_LEN) {
...
#else
          NXPLOG_UWB_TML_D(
              "Empty packet Read, Ignore read and try new read...");
#endif
          phOsalUwb_ProduceSemaphore(gpphTmlUwb_Context->rxSemaphore);
        } else {
          // Fira Test mode is enabled
          if (nxpucihal_ctrl.operationMode == kOPERATION_MODE_mctt) {
            ...
            if (UCI_MT_RSP ==
                ((temp_payload[0] & UCI_MT_MASK) >> UCI_MT_SHIFT)) {
              // Response packet received
              pbf = (temp_payload[0] & UCI_PBF_MASK) >> UCI_PBF_SHIFT;
              if ((pbf == TRUE) && (chain_packet_counter == 0)) {
                // Chained packet start. Copy to pBuffer and continue normal
                // flow
                phOsalUwb_MemCopy(gpphTmlUwb_Context->tReadInfo.pBuffer,
                                  temp_payload, (uint32_t)dwNoBytesWrRd);
                ...
                if (gpphTmlUwb_Context->appDataCallback != NULL) {
                  ...
                  gpphTmlUwb_Context->appDataCallback(
                      temp_payload, (uint16_t)(dwNoBytesWrRd));
                  ...
                  // Produce semaphore as data has already been sent to
                  // appDataCallback
                  phOsalUwb_ProduceSemaphore(gpphTmlUwb_Context->rxSemaphore);
                  continue;
                } else {
              ...
              if (gpphTmlUwb_Context->appDataCallback != NULL) {
                ...
                gpphTmlUwb_Context->appDataCallback(temp_payload,
...
#if UWBIOT_UWBD_SR040
          phTmlUwb_PrintRecevedMessage(temp_payload,
                                       gpphTmlUwb_Context->tReadInfo.wLength);
#else
          if (nxpucihal_ctrl.operationMode != kOPERATION_MODE_mctt) {
            if (gpphTmlUwb_Context->tReadInfo.wLength > 200) {
              LOG_RX("RECV ", temp_payload, 20);
            } else {
              LOG_RX("RECV ", temp_payload,
                     gpphTmlUwb_Context->tReadInfo.wLength);
            }
          }
#endif
...
static void phTmlUwb_PrintRecevedMessage(const uint8_t* const pBuffer,
                                         const uint16_t wLength) {
#if defined(DEBUG) && UWBIOT_UWBD_SR040 && SR040_PRINT_DEBUG_LOG_MESSAGES && \
    defined(_MSC_VER)
  if ((wLength >= 5)                       /* */
      && (pBuffer[0] == 0x6E)              /* */
      && (pBuffer[1] == 0x00)              /* */
...
static void phTmlUwb_CleanUp(void) {
  ...
  if (NULL != gpphTmlUwb_Context->pDevHandle) {
    ...
    /* Add delay to allow all async tasks (Threads) to exit */
    phOsalUwb_Delay(100);
...
UWBSTATUS phTmlUwb_Write(uint8_t* p_data, uint16_t data_len) {
  ...
  /* Check whether TML is Initialized */
  if (NULL != gpphTmlUwb_Context) {
    if ((NULL != gpphTmlUwb_Context->pDevHandle) && (NULL != p_data) &&
        (PH_TMLUWB_RESET_VALUE != data_len)) {
  ...
  if (NULL != gpphTmlUwb_Context) {
    if ((gpphTmlUwb_Context->pDevHandle != NULL) && (NULL != pBuffer) &&
      ...
      if (!gpphTmlUwb_Context->tReadInfo.bThreadBusy) {
        ...
        phOsalUwb_ProduceSemaphore(
            gpphTmlUwb_Context->rxSemaphore);  // To be enabled later
...
void phTmlUwb_DeferredCall(uintptr_t dwThreadId,
  ...
  if (phOsalUwb_ConsumeSemaphore_WithTimeout(
          gpphTmlUwb_Context->postMsgSemaphore,
          PH_TML_UWB_MAX_MESSAGE_POST_WAIT) != UWBSTATUS_SUCCESS) {
    NXPLOG_UWB_TML_E("phTmlUwb_DeferredCall: consume semaphore error");
    (void)phOsalUwb_ProduceSemaphore(gpphTmlUwb_Context->postMsgSemaphore);
    return;
  }
  (void)phOsalUwb_msgsnd(gpphTmlUwb_Context->dwCallbackThreadId, ptWorkerMsg,
                         NO_DELAY);
  (void)phOsalUwb_ProduceSemaphore(gpphTmlUwb_Context->postMsgSemaphore);
}
