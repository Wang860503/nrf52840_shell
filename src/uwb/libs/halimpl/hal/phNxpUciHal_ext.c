 ...
 *
 ******************************************************************************/
static UWBSTATUS phNxpUciHal_process_ext_cmd_rsp(uint16_t cmd_len,
                                                 uint8_t* p_cmd) {
  ...
  do {
    ...
    /* Start timer */
    status =
        phOsalUwb_Timer_Start(uwbTimeoutTimerId, HAL_EXTNS_WRITE_RSP_TIMEOUT,
    ...
    /* timer already started with HAL_EXTNS_WRITE_RSP_TIMEOUT, wait for + 10
     * msec for semaphore. */
    if (phOsalUwb_ConsumeSemaphore_WithTimeout(
            nxpucihal_ctrl.ext_cb_data.sem, HAL_EXTNS_WRITE_RSP_SEM_TIMEOUT) !=
        UWBSTATUS_SUCCESS) {
...
UWBSTATUS phNxpUciHal_set_board_config() {
  UWBSTATUS status;
  uint8_t buffer[] = {(UCI_MTS_CMD | UCI_GID_PROPRIETARY),
  ...
  if (phNxpUciHal_GetNxpNumValue(UWB_BOARD_VARIANT_CONFIG, &boardConfig,
                                 sizeof(boardConfig))) {
    NXPLOG_UCIHAL_D("%s: UWB_BOARD_VARIANT_CONFIG: %x", __FUNCTION__,
                    boardConfig);
  } else
  ...
  /*
   * In Case of PNP, host PC is unaware of the target Board where pnp FW is
   * actually running This API help in fetching Board version from PNP FW
   */
...
#else
  if (phNxpUciHal_GetNxpNumValue(UWB_BOARD_VARIANT_VERSION, &boardVersion,
                                 sizeof(boardVersion))) {
    NXPLOG_UCIHAL_D("%s: UWB_BOARD_VARIANT_VERSION: %x", __FUNCTION__,
                    boardVersion);
  } else
...
UWBSTATUS phNxpUciHal_dump_fw_crash_log() {
  UWBSTATUS status;
  uint8_t cmd_buff[] = {(UCI_MTS_CMD | UCI_GID_PROPRIETARY),
                        UCI_DBG_GET_ERROR_LOG_CMD, 0x00, 0x00};
  uint8_t cmd_len;
