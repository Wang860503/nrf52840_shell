...
#define APP_DATA_MAX_SIZE \
  0x74 /* Max Applicaiton data trasnfer size 116 bytes as per UCI Spec*/
...
#define UCI_MAX_DATA_PACKET_SIZE                                              \
  4200 /* Max Radar Notificaiton Receive from the UWBS is 4106 Payload and 14 \
          Bytes header for Bulk Transfer */
...
#define UCI_MSG_BLD_HDR1_EXT(p, oid) \
  *(p)++ = (uint8_t)(((oid) << UCI_OID_SHIFT) | (UCI_EXT_MASK));
...
#define UCI_GET_CMD_BUF(paramlen)                                             \
  ((UWB_HDR*)phOsalUwb_GetMemory((uint16_t)(UWB_HDR_SIZE + UCI_MSG_HDR_SIZE + \
                                            UCI_MSG_OFFSET_SIZE +             \
                                            (paramlen))))
 ...
 */
#define MAX_RADAR_LEN \
  4106  // MAX 8 CIRS * 128 CIR TAPS *4 (1 TAP = 4 BYTES) + 10 CIR
        // HEADER(exclude UCI Header)
#define MAX_RESPONSE_DATA_DLTDOA_TAG 472
#define MAX_RESPONSE_DATA_DEBUG_NTF 4106  // For CIR, PSDU debug Notification
#define MAX_RESPONSE_DATA_RCV \
  2031  // For Data transfer, max data size which we can send and rcv is 2031
#define MAX_RESPONSE_DATA_DATA_TRANSFER \
  2048  // For Data transfer, max data size which we can send and rcv is 2031 +
        // 16 (data header)
...
#define UCI_MAX_CMD_BUF_LEN \
  (UWB_HDR_SIZE + UCI_MSG_HDR_SIZE + MAX_CMD_BUFFER_DATA_TRANSFER)
#else
...
#endif
  UWBD_STATUS_READY = 0x01, /* UWBD is ready for  performing uwb session with
                               non SE use cases */
